let ground,lander;
var lander_img,bg_img,trust,ras_left,rcs_right
var vx=0,vy=0,g=0.05,f=100;
...
thrust.playing= true; thrust.looping= false;
...
vy +=g; lander.position.y+=vy; lander.position.x +=vx;
drawSprites
function keyPressed() { if(keyCode==UP_ARROW && fuel>0) { upward_thrust(); 
    lander.changeAnimation('thrusting'); 
    thrust.nextFrame();
 } if(keyCode==RIGHT_ARROW && fuel>0) { lander.changeAnimation('left');
  right_thrust(); }
  if(keyCode==LEFT_ARROW && fuel>0) { lander.changeAnimation('right'); left_thrust(); } }
function upward_thrust() { vy = -1; fuel-=1; }
function right_thrust() { vx += 0.2; fuel -=1; }
function left_thrust() { vx -= 0.2; fuel-=1; }